...
| P1-2 | **最微妙逻辑零内联测试 + 无属性测试** | `src/extract.rs` 577 行 / 0 测试 | 为 `extract_undeclared` 加 proptest，断言不变量"判为可选 ⇒ 严格模式下确实可缺省渲染" | ✅ 已修复（18 项样例测试 + 300 例属性测试；未引入 proptest，用确定性生成器替代） |
...
> `extract.rs` 测试（18 项样例 + 300 例属性测试，用确定性生成器替代 proptest）、
> 公开 `render` 的有限性闸门；
