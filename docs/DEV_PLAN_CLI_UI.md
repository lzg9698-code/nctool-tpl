...
>
> ⚠️ **状态注记（2026-09-22）**：本文写于项目早期，**§1「现状」与 §7「阶段计划」均已过时**，
> 且 §7 已被 `docs/ROADMAP.md` 取代。文中「245 项测试 / v0.3.1 / v0.2.0 / Web UI 尚未开始」
> 等描述**不再反映当前状态**（现为 571 项测试 / tpl 0.4.0 / core 0.3.0 / cli 0.3.0 /
> Web UI 完整闭环）。**现状请查 `docs/PROJECT_STATUS.md`，规划请查 `docs/ROADMAP.md`。**
> 本文的命令面设计、API 契约、技术决策仍具参考价值。
...
> ⚠️ **本节已被 [docs/ROADMAP.md](ROADMAP.md) 取代**（2026-09-03）。
>
> 旧计划遗漏了两件事：① **工艺正确性验证**——G-code 未经评审即投产存在撞刀风险；
> ② **代码债务处置**——`ui/` 目录未纳入版本控制、CHANGELOG `[未发布]` 段积压未发版。
>
> ROADMAP 按「需求与设计收口 → 基础架构稳固 → UI 服务联通 → 完整交互闭环 → 联调测试 → 上线与迭代」
> 重新划分阶段，并将阶段 3（UI 完整交互）降级为优先推迟项。
>
> 本节的**现状分析、架构设计、技术决策、命令面、API 契约**仍然有效，继续作为设计输入。
> 另：本节记录的 245 项测试已过期，实测为 279 项（2026-09-03 阶段 A 收口后）。
