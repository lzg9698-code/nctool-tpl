...
| **B** 基础架构稳固 | 还清债务，地基干净 | P0 | 3–4 | **13/13** | — | ✅ 完成（B4.2 于 2026-09-22 步骤化） |
...
| **合计** | | | **26–40** | **71/72 (99%)** | 仅剩 A1（外部依赖） | ≈99% |
...
B1–B5 全部完成。B4.2（golden 变更保护步骤化）已于 2026-09-22 完成：`scripts/refresh_golden.sh`（CI 拒刷 + 数量守卫 + diff 概览）+ Rust 侧 `assert!(CI.is_none())` + CI 自测步骤；流程写入 `CONTRIBUTING.md`。
...
| 文件 | 行覆盖（生产口径） | 说明 |
|---|---|---|
| `cli/src/commands/ui.rs` | **20.00%** | 全项目最低（15 行生产代码仅 3 行覆盖）—— 子进程启动路径难以单测 |
| `cli/src/commands/inspect.rs` | 80.79% | 输出层 |
| `core/src/variables.rs` | 83.70% | 变量提取 |
| `cli/src/context.rs` | 84.35% | 模板/机床解析入口 |
| `core/src/pipeline.rs` | 84.74% | 生成管线 |
| `cli/src/server.rs` | 85.40% | HTTP 服务层 |
...
- `ui/index.html` **2735 行** ×2 副本（md5 一致），上限 3000 行 → **余量收窄至 265 行**（见 §6.2 R3）。
...
| B4.2 | ~~golden 变更保护步骤化（`NCTOOL_UPDATE_GOLDEN` 人工流程文档化）~~ | ✅ 已清（2026-09-22，A6）：新增 `scripts/refresh_golden.sh`（CI 拒刷 + 数量守卫 + diff 概览）+ CI 自测步骤；流程写入 `CONTRIBUTING.md` |
...
| R3 | 单文件前端失控（3000 行上限） | — | 🟡 **余量收窄**：`ui/index.html` 2081（09-10）→ **2735 行**（2026-09-22 实测），上限 3000，余量 265 行。新增 UI 功能前应评估拆分 |
...
| 2 | 参数集命名预设 | 34 | ✅ 已实现（Web UI：`localStorage` 的 `nctool.presets` + 预设 chips，支持保存/应用/删除，便捷键 `⌘/Ctrl+S`） |
| 4 | 浏览器内模板编辑 | 21 | ⬜ 服务模式已明确禁用（`toast("服务模式暂不支持浏览器内保存模板")`）；需写 API + 路径穿越/并发覆盖防护 |
| 5 | i18n | 20 | ⬜ 取决于商业交付决策 |
| — | ~~B4.2 golden 保护步骤化~~ / P2-2~P2-6 架构评审剩余项 / F1 转速联动 / F4 机键 / F2 组合模板 | — | B4.2 ✅ 已清（A6）；余见 §6.1 与 `PROCESS_CHECKLIST.md` |
...
**第一优先：第四轮审查批次 E 的 UI 两项**

`docs/CODE_REVIEW_2026-09-19.md` §4 的批次 A/B/C/D/E/F 已落地（批次 D 的 P1-10 / P1-11
于 2026-09-22 收口），仅剩批次 E 的 UI 两项：

- **P2-25** Bool 参数恒提交 `false`（与 CLI 省略该键 → 条件必选判定可能分歧）。
- **P2-26** 无 `AbortController`（后端阻塞时旧请求堆积）。

两者均需浏览器验证，属前端改动。

> **历史项已关闭（2026-09-22 核对）**：第三轮的 UI 存储型 XSS（P1-1）已于 2026-09-19
> 修复（`de64e32`）；P1-4（`TemplateMeta::default()` 与 serde 默认值不一致）已在批次 A 收口；
> 批次 D 的 P1-10（CLI 读文件上限）与 P1-11（错误链）已于 2026-09-22 修复。

**第二优先：补覆盖率洼地**（✅ 已完成，2026-09-22）
