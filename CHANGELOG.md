...
"NC 数值格式化"版本：新增 G-code 专用数值格式化过滤器，支撑真实坐标/程序号输出。
...
- **测试**：8 项 NC 过滤器测试（小数位/去尾零/前导零/整数输入/负数/非有限数/零宽度报错/G-code 组合场景），总测试 78 → 86 项
...
- 从 78 项增至 86 项（68 单元 + 17 集成 + 1 文档）
- 新增覆盖：nc_fixed / nc_strip / nc_pad 的常规与异常输入
