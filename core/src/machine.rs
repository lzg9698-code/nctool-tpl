...
pub const KNOWN_CONFIG_KEYS: &[MachineKeySchema] = &[
    MachineKeySchema { key: "program_prefix", kind: MachineKeyKind::String, default: "O", description: "程序号前缀（FANUC 风格 O/Mazak P 等）" },
    MachineKeySchema { key: "program_digits", kind: MachineKeyKind::Integer, default: "4", description: "程序号位数（前导零填充）" },
    MachineKeySchema { key: "line_number_prefix", kind: MachineKeyKind::String, default: "N", description: "行号前缀" },
    MachineKeySchema { key: "line_number_digits", kind: MachineKeyKind::Integer, default: "4", description: "行号位数" },
    MachineKeySchema { key: "coordinate_system", kind: MachineKeyKind::String, default: "G54", description: "工件坐标系（G54–G59）" },
    MachineKeySchema { key: "units", kind: MachineKeyKind::Choice(&["metric", "imperial"]), default: "metric", description: "单位制（metric→G21，imperial→G20）" },
    MachineKeySchema { key: "feed_mode", kind: MachineKeyKind::Choice(&["G94", "G95"]), default: "G94", description: "进给模式（G94 每分进给 / G95 每转进给）" },
    MachineKeySchema { key: "rapid", kind: MachineKeyKind::String, default: "G0", description: "快速移动 G 码" },
    MachineKeySchema { key: "linear", kind: MachineKeyKind::String, default: "G1", description: "线性插补 G 码" },
    MachineKeySchema { key: "clockwise_arc", kind: MachineKeyKind::String, default: "G2", description: "顺时针圆弧 G 码" },
    MachineKeySchema { key: "ccw_arc", kind: MachineKeyKind::String, default: "G3", description: "逆时针圆弧 G 码" },
    MachineKeySchema { key: "spindle_on", kind: MachineKeyKind::String, default: "M3", description: "主轴正转 M 码" },
    MachineKeySchema { key: "spindle_off", kind: MachineKeyKind::String, default: "M5", description: "主轴停止 M 码" },
    MachineKeySchema { key: "coolant_on", kind: MachineKeyKind::String, default: "M8", description: "冷却开 M 码" },
    MachineKeySchema { key: "coolant_off", kind: MachineKeyKind::String, default: "M9", description: "冷却关 M 码" },
    MachineKeySchema { key: "program_end", kind: MachineKeyKind::String, default: "M30", description: "程序结束 M 码（M30/M99 等）" },
    MachineKeySchema { key: "tool_change", kind: MachineKeyKind::String, default: "M6", description: "换刀 M 码" },
    MachineKeySchema { key: "max_spindle_rpm", kind: MachineKeyKind::Integer, default: "6000", description: "主轴最高转速（供文档与用户自检，模板可引用）" },
    MachineKeySchema { key: "machine_type", kind: MachineKeyKind::String, default: "generic", description: "机床类型标识（generic / mill_turn_multitask 等）" },
    // 扩展键：无通用默认值，仅特定预设使用
    MachineKeySchema { key: "axes", kind: MachineKeyKind::String, default: "", description: "轴配置描述（如 \"X Z C / X Y Z B C\"，文档用途）" },
];
